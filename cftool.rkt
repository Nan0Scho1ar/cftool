...
;; This will create:
;; 1. A function for creating the new types.
;; 2. A hash-map for storing registered types
;; 3. A function to register types (store type instance in hash-map)
;; (define-syntax-rule (define-cf-item name collection-name fn-name)
;;   (begin
;;     (define name (make-keyword-procedure ((curry cf-item->hash) 'name)))
;;     (define collection-name (make-hash))
;;     (define (fn-name item) (hash-set! collection-name (hash-ref item 'id) item))))

;; Define cf-item types, collections, and registration-functions
;; (define-cf-item aws-credential   credentials   register-credential)
...
(define (stack-template
         ...
         #:depends-on [dependencies null]
         #:pre-create-fn [pre-create-fn null]
         #:post-create-fn [post-create-fn null]
         #:pre-update-fn [pre-update-fn null]
         #:post-update-fn [post-update-fn null]
         #:pre-delete-fn [pre-delete-fn null]
         #:post-delete-fn [post-delete-fn null]
         #:create-warning [create-warning null]
         #:update-warning [update-warning null]
         #:delete-warning [delete-warning null]
